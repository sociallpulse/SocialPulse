        ...
        <div className="p-4 sm:p-8 space-y-6 sm:space-y-8 max-w-7xl mx-auto">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
                ...
                <div className="bg-white rounded-[2rem] sm:rounded-[2.5rem] border border-slate-200 shadow-sm p-6 sm:p-8 flex flex-col max-h-[500px]">
                   <h3 className="text-base sm:text-lg font-black text-slate-800 mb-6 flex items-center justify-between">
                      ...
                      <span className="text-[10px] bg-slate-100 px-2 py-1 rounded-lg text-slate-500">Live</span>
                   </h3>
                    ...
                    {profiles.map((profile) => (
                      <tr key={profile.id} className="hover:bg-indigo-50/30 transition-all">
                        <td className="px-10 py-7 flex items-center gap-4">
                          <div className="w-12 h-12 rounded-xl bg-indigo-600 flex items-center justify-center font-black text-white">{profile.handle.charAt(0).toUpperCase()}</div>
                          <div>
                            <span className="font-black text-slate-800" dir="ltr">@{profile.handle}</span>
                            <p className="text-[10px] text-slate-400 mt-1">متن‌محور (بدون تصویر)</p>
                          </div>
                        </td>
              ...
              <div className="sm:hidden divide-y divide-slate-100">
                {profiles.map((profile) => (
                  <div key={profile.id} className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-indigo-600 text-white flex items-center justify-center font-black">{profile.handle.charAt(0).toUpperCase()}</div>
                      <div>
                ...
                <div className="bg-[#0F172A] rounded-[2rem] sm:rounded-[3.5rem] p-8 sm:p-16 text-white relative overflow-hidden shadow-2xl">
                   <div className="relative z-10">
                      ...
                      <p className="text-sm sm:text-lg text-slate-400 font-medium leading-relaxed mb-8">
                         تجمیع تمام اسناد متنی استخراج شده یک فرد و بررسی تغییرات الگوهای رفتاری توسط مدل زبانی Gemini.
                      </p>
                      <button className="w-full sm:w-auto px-8 py-4 bg-indigo-600 text-white rounded-xl font-black shadow-xl hover:bg-indigo-700 transition-all flex items-center justify-center gap-3">
                         <BrainCircuit className="w-5 h-5" /> شروع پردازش عمیق (Deep Scan)
                      </button>
                   </div>
                </div>
             </div>
          ...
          {(activeTab === 'team' || activeTab === 'logs') && (
            <div className="bg-white rounded-[2rem] border border-slate-200 p-12 text-center">
               ...
               <h3 className="text-lg font-black text-slate-800">در حال توسعه</h3>
               <p className="text-sm text-slate-400 mt-2 font-medium">این بخش در بروزرسانی بعدی سیستم در دسترس قرار خواهد گرفت.</p>
            </div>
          )}
