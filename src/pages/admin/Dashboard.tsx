...
import { 
  ...
  CheckCircle2, UserPlus, ShieldAlert, Trash2, AlertTriangle, PlayCircle
} from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // دریافت آخرین تحلیل‌ها با جزئیات بیشتر
      ...
      // آمار و شناسایی موارد گیر کرده (بیش از ۱۰ دقیقه در وضعیت Pending)
      ...
      // رفع خطای Property 'status' does not exist on type 'never' با کست کردن به any[]
  ...
  // تابع بازنشانی موارد گیر کرده (به صورت دستی اگر وب‌هوک عمل نکرد)
  const handleResetStuckItems = async () => {
    if (window.confirm("آیا می‌خواهید وضعیت موارد گیر کرده را به 'failed' تغییر دهید تا صف آزاد شود؟")) {
      const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000).toISOString();
            ...
            <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 animate-fade-in pb-12">
              ...
              {stats.stuckItems > 0 && (
                <div className="bg-amber-50 border border-amber-200 rounded-3xl p-4 flex items-center justify-between">
                   <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-black text-amber-900">اختلال در صف پردازش شناسایی شد</p>
                        <p className="text-xs text-amber-700 font-bold">{stats.stuckItems} سند بیش از ۱۰ دقیقه در وضعیت انتظار باقی مانده‌اند.</p>
                      </div>
                   </div>
                   <button onClick={() => setActiveTab('intelligence')} className="text-xs font-black bg-amber-900 text-white px-4 py-2 rounded-xl shadow-lg">بررسی واحد هوشمند</button>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6">
                {[
                  ...
                  <div key={idx} className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm group">
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 sm:gap-8">
                <div className="xl:col-span-2 bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col p-8">
                    ...
                    <div className="flex items-end justify-between h-48 gap-4 mt-auto">
                      {[40, 85, 55, 100, 75, 90, 60].map((h, i) => (
                        <div key={i} className="flex-1 flex flex-col items-center group cursor-pointer">
                          ...
                          <span className="text-[10px] font-black text-slate-400 mt-4 uppercase">
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-6 sm:p-8 flex flex-col max-h-[600px]">
                   <h3 className="text-base font-black text-slate-800 mb-6 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <Activity className="w-5 h-5 text-indigo-500" /> وقایع زنده
                      </div>
                   </h3>
                        ...
                        <div key={sub.id} className="p-4 border border-slate-50 bg-slate-50/30 rounded-2xl flex gap-4 hover:bg-slate-50 transition-colors">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[11px] text-slate-500 font-medium line-clamp-1 italic">
                              {sub.status === 'completed' ? sub.summary : 'در حال بررسی توسط هسته هوشمند...'}
                            </p>
                          </div>
                        </div>
                      ))}
            ...
            <div className="max-w-7xl mx-auto bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden animate-slide-up flex flex-col">
              <div className="p-6 sm:p-10 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-6 bg-slate-50/50">
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-bold mt-1">مدیریت آیدی‌های شناسایی شده</p>
                </div>
                <div className="relative group w-full sm:w-80">
                  <Search className="w-5 h-5 absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-600" />
                  <input 
                    type="text" 
                    placeholder="جستجو..." 
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-8 sm:p-12">
                   <div className="flex items-center justify-between mb-8">
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                            <h3 className="text-xl font-black text-slate-800">وضعیت صف پردازش زنده</h3>
                            <p className="text-xs text-slate-400 font-bold mt-1">مانیتورینگ اسناد در انتظار هوش مصنوعی</p>
                         </div>
                      </div>
                      <div className="text-right">
                         <p className="text-2xl font-black text-amber-600">{stats.pendingItems}</p>
                         <p className="text-[10px] font-black text-slate-400 uppercase">Items Waiting</p>
                      </div>
                   </div>

                   <div className="space-y-3">
                      {recentSubmissions.filter(s => s.status === 'pending' || s.status === 'failed').length === 0 ? (
                        <div className="py-12 text-center bg-slate-50 rounded-3xl border border-dashed border-slate-200">
                           <CheckCircle2 className="w-10 h-10 text-emerald-400 mx-auto mb-4" />
                           <p className="text-sm font-black text-slate-500">تمامی اسناد با موفقیت پردازش شده‌اند.</p>
                        </div>
                      ) : (
                        recentSubmissions.filter(s => s.status === 'pending' || s.status === 'failed').map((s) => (
                           <div key={s.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between">
                              <div className="flex items-center gap-4">
                                 <div className={`w-2 h-2 rounded-full ${s.status === 'pending' ? 'bg-amber-500 animate-pulse' : 'bg-red-500'}`}></div>
                                 <div>
                                    <p className="text-xs font-black text-slate-700" dir="ltr">Submission ID: #{s.id.slice(0, 12)}</p>
                                    <p className="text-[10px] text-slate-400 font-bold mt-1">ثبت شده در: {new Date(s.created_at || '').toLocaleTimeString('fa-IR')}</p>
                                 </div>
                              </div>
                              <div className="flex items-center gap-3">
                ...
                <div className="bg-[#0F172A] rounded-[3rem] p-10 sm:p-16 text-white relative overflow-hidden shadow-2xl">
                   <div className="relative z-10">
                      <div className="inline-flex items-center gap-2 bg-white/10 px-4 py-2 rounded-2xl border border-white/10 mb-8 backdrop-blur-md">
                         ...
                         <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-300">Advanced Engine</span>
                      </div>
                      <h2 className="text-3xl sm:text-5xl font-black mb-6 leading-tight">اسکن عمیق (Deep Scan)</h2>
                      <p className="text-sm sm:text-lg text-slate-400 font-medium leading-relaxed mb-10 max-w-2xl">
                         تحلیل الگوی رفتاری کل تاریخچه یک پروفایل توسط مدل Gemini 2.5.
                      </p>
                      <div className="flex flex-col sm:flex-row gap-4 max-w-xl">
                        ...
                        <button className="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-xl hover:bg-indigo-700 transition-all flex items-center justify-center gap-3">
            ...
            <div className="max-w-7xl mx-auto bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden animate-slide-up flex flex-col">
               <div className="p-6 sm:p-10 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-6 bg-slate-50/50">
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-bold mt-1">تغییر سطوح دسترسی و نظارت بر پرسنل</p>
                </div>
                <button className="flex items-center justify-center gap-2 bg-slate-900 text-white px-6 py-3.5 rounded-2xl text-sm font-bold shadow-lg hover:bg-black transition-all">
                  <UserPlus className="w-4 h-4" /> دعوت همکار
                </button>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-right border-collapse min-w-[750px]">
                  <thead className="bg-slate-50 text-slate-400 text-[10px] font-black uppercase tracking-widest border-b border-slate-100">
                    <tr>
                      <th className="px-8 py-5">اطلاعات رصدگر</th>
                      <th className="px-8 py-5 text-center">شناسه Personnel</th>
                      ...
                      <tr key={member.user_id} className="hover:bg-slate-50/50 transition-all">
                        ...
                        <td className="px-8 py-6 text-center">
                          <button 
                            ...
                            className="text-[10px] font-black px-4 py-2 rounded-xl bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all shadow-sm"
                          >
                            تغییر نقش
                          </button>
                        </td>
          ...
          {activeTab === 'logs' && (
            <div className="max-w-5xl mx-auto bg-slate-900 rounded-[2.5rem] border border-slate-800 p-8 sm:p-12 text-emerald-400 font-mono text-xs shadow-2xl relative">
               <div className="flex items-center gap-3 mb-8 border-b border-white/10 pb-6">
                  ...
                  <h3 className="text-base font-black text-white">SYSTEM SECURITY CONSOLE</h3>
               </div>
               <div className="space-y-4">
                 <p><span className="text-slate-500">[{new Date().toLocaleTimeString()}]</span> <span className="text-blue-400">[AUTH]</span> Admin session verified.</p>
                 <p><span className="text-slate-500">[{new Date().toLocaleTimeString()}]</span> <span className="text-purple-400">[DB]</span> Real-time WebSocket established.</p>
                 <p><span className="text-slate-500">[{new Date().toLocaleTimeString()}]</span> <span className="text-amber-400">[AI]</span> Gemini Flash 2.5 engine standby.</p>
                 <div className="mt-10 pt-6 border-t border-white/5 flex items-center gap-2 animate-pulse">
                    <div className="w-2 h-2 rounded-full bg-emerald-500"></div>
                    <span className="text-slate-500 italic">Watching for background events...</span>
                 </div>
               </div>
            </div>
          )}
